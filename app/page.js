...
export default function Home() {
  ...
  const uploadFiles = async () => {
    ...
    try {
      const response = await fetch('https://spotifydashboardbackend-production-e5f8.up.railway.app/upload/', {
        method: 'POST',
        body: formData
      });
      if (!response.ok) {
  ...
  const fetchMetrics = async (id) => {
    try {
      const url = new URL(`https://spotifydashboardbackend-production-e5f8.up.railway.app/metrics/${id}`);
